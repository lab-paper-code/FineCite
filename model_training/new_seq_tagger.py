...
CUDA_VISIBLE_DEVICES=1 nohup python3 -u new_seq_tagger.py --model_name scibert --segment token_scibert --mode scopes --learning_rate 6e-05 --dropout 0.06 --batch_size 2 --no_background > output/scibert/log.txt
...
CUDA_VISIBLE_DEVICES=2 nohup bash -c 'for seg in sentence_prio sentence_majo ;
        do
            for md in scopes total;
            do
                python3 -u new_seq_tagger.py --model_name llm2vec_mistral --segment {seg} --mode {md} --learning_rate 5e-5 --dropout 0.1 --batch_size 2;
            done;
        done;
    done;
done'> output/llm2vec_mistral/log.txt

CUDA_VISIBLE_DEVICES=1 nohup bash -c 'for seg in sentence_prio sentence_majo ;
        do
            for md in scopes total;
            do
                python3 -u new_seq_tagger.py --model_name llm2vec_llama3 --segment {seg} --mode {md} --learning_rate 5e-5 --dropout 0.1 --batch_size 2;
            done;
        done;
    done;
done'> output/llm2vec_llama3/log.txt

CUDA_VISIBLE_DEVICES=2 nohup bash -c 'for seg in sentence_majo ;
        do
            for md in total;
            do
                python3 -u new_seq_tagger.py --model_name scibert --segment {seg} --mode {md} --learning_rate 5e-5 --dropout 0.1 --batch_size 2;
            done;
        done;
    done;
done'> output/scibert/log.txt
...
---> Hyperparameter tuning for Mistral
...
---> Hyperparameter tuning for LLaMA3
